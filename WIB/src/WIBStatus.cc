...
void WIBTool::WIBStatus::ProcessFEMB(uint8_t FEMB){  
  ...
  // reset register
  wib->WriteWithRetry(0x05,0x0); // changed

  // check if board is on
  if( (g->mask & wib->ReadWithRetry(g->address))==g->mask ) FEMB_PWR[iFEMB]=1; // changed
  ...
  // get temp and Vcc
  wib->WriteWithRetry(pwrMesSel,wib->GetItem(pwrMesSelBrd+"_VCC_TEMP")->mask); // changed
  ...
  */
  ...
  uint32_t upperBits = (wib->ReadWithRetry("PWR_MES_OUT_V")); // changed
  uint32_t lowerBits = ConvertSignedInt(wib->ReadWithRetry("PWR_MES_OUT_C_TEMP")); // changed
  ...
  for(int i=0; i<6; i++){
    ...
    FEMB_C[iFEMB][i]=0;
    uint8_t iv = i+1;
    wib       ->WriteWithRetry(pwrMesSel,wib->GetItem(pwrMesSelBrd+"_"+std::to_string(iv))->mask); // changed
    //upperBits = ConvertSignedInt(wib->Read("PWR_MES_OUT_V"));
    upperBits = (wib->ReadWithRetry("PWR_MES_OUT_V")); // changed
    lowerBits = ConvertSignedInt(wib->ReadWithRetry("PWR_MES_OUT_C_TEMP")); // changed
  ...
  uint32_t linkStatBits = wib->ReadWithRetry("LINK_SYNC_STATUS_BRD"+std::to_string(FEMB)); // changed
  ...
  uint32_t eqStatBits = wib->ReadWithRetry("EQ_LOS_BRD"+std::to_string(FEMB)+"_RX"); // changed
  ...
  wib->WriteWithRetry("FEMB_STAT_SEL",uint32_t(iFEMB)); // changed
  for(int i=0; i<4; i++){
    wib->WriteWithRetry("LINK_STAT_SEL",uint32_t(i)); // changed
    CHKSUM_ERROR_COUNT[iFEMB][i]  = wib->ReadWithRetry("CHKSUM_ERROR"); // changed
    TIME_STAMP[iFEMB][i]          = wib->ReadWithRetry("TIME_STAMP"); // changed
    TS_ERROR_COUNT[iFEMB][i]      = wib->ReadWithRetry("TS_ERROR"); // changed 
    FRAME_ERROR_COUNT[iFEMB][i]   = wib->ReadWithRetry("FRAME_ERROR"); // changed 
  }
...
void WIBTool::WIBStatus::ProcessWIB(){  
  ...
  // Vcc and temp
  wib->WriteWithRetry(pwrMesSel,wib->GetItem(pwrMesSel+"_WIB_VCC_TEMP")->mask); // changed
  ...
  for(int i=0; i<4; i++){
    ...
    WIB_C[i]=0;
    uint8_t iv = i+1;
    wib       ->WriteWithRetry(pwrMesSel,wib->GetItem((pwrMesSel+"_WIB_"+std::to_string(iv)).c_str())->mask); // changed
    upperBits = (wib->ReadWithRetry("PWR_MES_OUT_V")); // changed
    lowerBits = ConvertSignedInt(wib->ReadWithRetry("PWR_MES_OUT_C_TEMP")); // changed
  ...
  FEMB_CLK      = wib->ReadWithRetry("FEMB_CLK_SEL"); // changed
  FEMB_CMD      = wib->ReadWithRetry("FEMB_CMD_SEL"); // changed
...
void WIBTool::WIBStatus::StartPowerMes(){
  // disable filter
  wib->WriteWithRetry("FILTER_EN",0x0); // changed
  // repeatedly toggle bit 16 to enable LTC2991 CMS conversion
  for(int i=0; i<100; i++){
    wib->WriteWithRetry("PWR_MES_START",0x0); // changed
    wib->WriteWithRetry("PWR_MES_START",0x1); // changed 
  }
