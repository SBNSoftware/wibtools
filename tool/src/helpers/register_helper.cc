...
CommandReturn::status WIBTool::RegisterHelper::Read(std::vector<std::string> strArg,
  ...
  if(numericAddr){
    ...
    for(size_t iName = 0; iName < names.size();iName++){
      if(names[iName].size() > size_t(max_name_length)){
	max_name_length = names[iName].size();
      }
    }
    // Don't allow very long names to mess up the display
    if(max_name_length > 60){
      max_name_length = 60;
    }
    ...
    for(size_t iName = 0; iName < names.size();iName++){
      ...
      if(!skipPrintZero || (val != 0)){
	printf("%*s%s: 0x%08X\n",max_name_length,"",names[iName].c_str(),val);
      }
    }
  }
